...
export function Libra({
      ...
      },
      {
        ...
        cond: () => {
          if (convStage !== 2) {
            ...
            roomContainsItem(places["Speare Hall"], "Another Love Letter")
          ) {
            convStage = 3;
            this.timesTalkedTo = 0;
            return false;
  ...
  class Paws extends NPC {
    messages = [
      ...
      {
        m: "I think it's a great plan, why don't you try it out!",
        cond: () => convStage == 5,
      },
      {
        ...
        cond: () => convStage === 6 && this.timesTalkedTo === 0, // TODO: also detect if Mingus is here
      },
      {
        m: "Well, I suppose I can tell you the one word that's been on my mind: INSERT PASSWORD HERE", // TODO: yeah
        cond: () => convStage === 6 && this.timesTalkedTo >= 1, // TODO: also detect if Mingus is here
      },
      {
  ...
  class Hamper extends Droppable {
    mingus;
    constructor() {
      super(
        "Hamper",
        "A container of future forbidden contents",
        {
          use: () => {
            withVar(setPlayerPlace, (playerPlace) => {
            if (convStage === 5 && !this.mingus) {
                if (!this.mingus) {
                    this.mingus = places["Speare"].npcs[0];
                    places["Speare"].npcs = [];
                } else {
  ...
  useEffect(() => {
    ...
    places["Squashbusters"].items.push(new Hamper());
  }, []);
}
