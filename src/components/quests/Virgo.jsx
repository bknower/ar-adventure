...
export function Virgo({
  ...
  class Maiden extends NPC {
    messages = [
      ...
      {
        m: "Thank you for showing me that the students of Husky Hunt Academy have hope. For your ardor and virtue, I wish to bestow you a name: Meritorious.",
      ...
      },
      {
        ...
        cond: () => this.timesTalkedTo >= 1 && withVar(setInventory, (inventory) => {
...
class CerebralScanner extends Droppable {
  ...
  constructor() {
    ...
    const artList = new Droppable(
        ...
        {
            view: () => " Curry, Stwest, Green Line, Meserve... And what shall I try next?"
        },
        true,
        "blob:https://imgur.com/18edde26-c1f4-4a4d-9e0b-f4758cf492a9"
    )

  const maidenArtifact = new Droppable(
            ...
            withVar(setPlaces, (places) => {
                return withVar(setPlayerPlace, (playerPlace) => {
                  if(playerPlace == "Meserve Hall") {
                        ...
                        return "The green line shakes and springs a second track out onto Huntington Ave. The train is barreling directly towards five business majors, pausing for traffic at each intersection. On the other track lies Joseph Aoun. Directly in front of you appears a lever – you sense that if you pull it, the train will divert to the other track, saving the business majors but killing our dear president. You waste no time in deciding, and immediately walk away, leaving the business majors to their fate. However, as soon as you turn around, the green line catches fire, stopping in its tracks and killing no one."
  ...
  useEffect(() => {
    ...
    places["Ryder Hall"].items.push(maidenArtifact);
  }, []);
}
